...
import { dreamify as url } from "@/api";
...
export default async function handler(req, res) {
    ...
    if (!validate(json)) {
        res.status(400).json({
            error: 'باید یک سبک و یک تصویر انتخاب کنید و حداقل ۸ حرف بنویسید.'
        });
    } else {

        try {
            let request = { prompt: { main_fa: json.prompt, main_en: "" }, style: json.style, orientation: json.orientation, image: json.image, format: json.format, weight: json.weight };
...
function validate(data) {
    if ((data.prompt.length > 8) && (data.style > 0) && data.image) { return true; }
    else {
        return false;
    }
}
